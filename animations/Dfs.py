"""
Draw a tree taken in input from "input.txt" and animate Depth-First Search on it.
"""
...
class Dfs(Scene):
    ...
    def construct(self):
        ...
        title = Title("DFS - Depth First Search")
        self.add(title)
    ...
    def dfs(self, i, rect, par=None):
        global nodes, edges, graph
        nodes[i].set_fill(BLUE, opacity=1)
        nodes[i].text.set_color(BLACK)
        oldRect = SurroundingRectangle(nodes[i], buff=0.1, color=GREEN)
        for j in graph[i]:
            if j != par:
                newRect = SurroundingRectangle(nodes[j], buff=0.1, color=GREEN)
                self.play(rect.animate.become(newRect), run_time=1)
                self.dfs(j, rect, i)
                self.play(rect.animate.become(oldRect), run_time=1)
